...
static gboolean pipe_io_handler(GIOChannel *io, GIOCondition cond, gpointer data)
{
	...
	s = g_io_channel_read_chars(io, (gchar *)&info, sizeof(signal_info), &len, &err);
	...
	__unset_recovery_mode((ptr + x)->pkgid, (ptr + x)->pkgtype);
	if (WIFSIGNALED(info.status) || WEXITSTATUS(info.status)) {
		send_fail_signal((ptr + x)->pkgid, (ptr + x)->pkgtype, (ptr + x)->args);
		DBG("backend[%s] exit with error", (ptr + x)->pkgtype);
	} else {
		DBG("backend[%s] exit", (ptr + x)->pkgtype);
	}
...
static void sighandler(int signo)
{
	...
	write(pipe_sig[1], &info, sizeof(signal_info));
}
