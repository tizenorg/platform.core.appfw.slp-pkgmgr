...
static const char *info_db_path;
static const char *info_db_journal_path;
static const char *cert_db_path;
static const char *cert_db_journal_path;
static const char *manifest_dir_path;
...
{
	return _initdb_load_directory(uid, manifest_dir_path,
			"/usr/bin/pkginfo --imd");
}
...
static void _get_paths(uid_t uid)
{
	...
	manifest_dir_path = tzplatform_getenv(
			_is_global(uid) ? TZ_SYS_RW_PACKAGES : TZ_USER_PACKAGES);
}

static void _remove_old_dbs(void)
{
