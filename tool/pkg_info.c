...
static int __pkg_list_cb (const pkgmgr_pkginfo_h handle, void *user_data)
{
	...
	if (info->uid != GLOBAL_USER) {
		printf("**List of Ui-Apps**\n");
		ret = pkgmgr_appinfo_get_usr_list(handle, PM_UI_APP, app_func, (void *)test_data, info->uid);
		if (ret < 0) {
			printf("pkgmgr_get_info_app() failed\n");
		}
		printf("**List of Svc-Apps**\n");
		ret = pkgmgr_appinfo_get_usr_list(handle, PM_SVC_APP, app_func, (void *)test_data, info->uid);
		if (ret < 0) {
			printf("pkgmgr_get_info_app() failed\n");
		}
	} else {
