...
static int __set_pkg_restriction_mode(pkgmgr_client *pc, const char *pkgid, int mode, uid_t uid)
{
	...
	if (pc == NULL || (mode != PM_RESTRICTION_MODE_UNINSTALL &&
			mode != PM_RESTRICTION_MODE_INSTALL)) {
		ERR("invalid parameter");
		return PKGMGR_R_EINVAL;
	}

	ret = comm_client_request(mpc->info.request.cc, "set_restriction_mode",
			g_variant_new("(uis)", uid, mode, pkgid), &result);
	if (ret != PKGMGR_R_OK) {
		ERR("request failed: %d", ret);
		return ret;
	}
...
{
	return __set_pkg_restriction_mode(pc, pkgid, uid, mode);
}
...
static int __get_pkg_restriction_mode(pkgmgr_client *pc, const char *pkgid, uid_t uid, int *mode)
{
	GVariant *result;
	int ret = PKGMGR_R_ECOMM;
	gint m;
...
{
	return __get_pkg_restriction_mode(pc, pkgid, uid, mode);
}
		...
		uid_t uid)
{
	...
	ret = comm_client_request(mpc->info.request.cc, "set_restriction_mode",
			g_variant_new("(uis)", uid, mode, "ALL"), &result);
	if (ret != PKGMGR_R_OK) {
		ERR("request failed: %d", ret);
		return ret;
	}
		...
		int *mode, uid_t uid)
{
	...
	ret = comm_client_request(mpc->info.request.cc,
			"get_restriction_mode",
			g_variant_new("(us)", uid, "ALL"), &result);
	if (ret != PKGMGR_R_OK) {
		ERR("request failed: %d", ret);
		return ret;
	}
