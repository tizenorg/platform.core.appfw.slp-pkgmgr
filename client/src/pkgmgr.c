		...
		const char *appid, pkgmgr_app_handler app_event_cb, uid_t uid)
{
	...
	result = comm_client_request(mpc->info.request.cc, "disable_global_app_splash_screen_for_uid",
			g_variant_new("(us)", uid, appid));
	if (result == NULL)
		return PKGMGR_R_ECOMM;
	g_variant_get(result, "(i&s)", &ret, &req_key);
		...
		const char *appid, pkgmgr_app_handler app_event_cb, uid_t uid)
{
	...
	result = comm_client_request(mpc->info.request.cc, "enable_global_app_splash_screen_for_uid",
			g_variant_new("(us)", uid, appid));
	if (result == NULL)
		return PKGMGR_R_ECOMM;
	g_variant_get(result, "(i&s)", &ret, &req_key);
